...
The order is a deliberate part of the execution model. It preserves invariants that a reader needs in order to reason about determinism and causality:
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Fresh estimates at PX4 ticks:} when the autopilot axis is due, PX4 always consumes an estimator packet \emph{computed from truth at the same boundary time} $t_k$.
  \item \textbf{Fresh telemetry at PX4 ticks:} battery and other algebraic telemetry are computed from the plant state at $t_k$ and published before stepping PX4, so uORB injection reflects the current plant state.
  \item \textbf{Command-to-physics causality:} the command published at $t_k$ is the unique held plant input over the next interval:
    \begin{equation}
      u(t) = u_k \quad \text{for } t\in[t_k,t_{k+1}),\qquad u_k \coloneqq \mathrm{PlantInput}(\mathrm{bus}(t_k)).
    \end{equation}
  \item \textbf{Snap-after-command for direct actuators:}\\ boundary-time discontinuities (e.g. \code{DirectActuators}) are applied \emph{after} \code{bus.cmd} is published, so snapped actuator state is a deterministic function of that same command.
...
Changing this ordering is not ``just refactoring'': it changes which signals are held over which intervals. For example, stepping PX4 before the estimator would make PX4 consume a stale estimate; logging before the autopilot stage would decouple recorded commands from the plant inputs that followed.
