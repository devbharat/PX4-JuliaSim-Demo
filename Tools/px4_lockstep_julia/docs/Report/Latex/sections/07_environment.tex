...
\end{align}
These correspond directly to \code{ground\_gap\_m} and \code{ground\_normal\_ned} (\code{src/sim/Contacts.jl}). Tangential quantities are taken in the ground plane: \(\vect{v}_{t}=(v_x,v_y)\), \(v_{t}=\|\vect{v}_{t}\|\).
...
\begin{equation}
  F_n = \max\!\bigl(0,\; k\,\delta + c\,\max(v_z,0)\bigr),
\end{equation}
...
\begin{equation}
  \vect{F}_t = -\mu F_n\,\frac{\vect{v}_{t}}{\|\vect{v}_{t}\| + v_{\epsilon}}.
\end{equation}
The resulting NED contact force applied at the COM is
\begin{equation}
  \vect{F}_{contact,ned} = (F_x, F_y, -F_n).
\end{equation}
Friction can be disabled. Because the friction law is smooth, it does not model true stiction: as \(\|\vect{v}_{t}\|\to 0\), \(\vect{F}_t \to \vect{0}\).
...
\paragraph{Continuous normal-reaction estimate (force-level evaluation).}
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
This cancels downward acceleration during resting contact (yielding \(\ddot{z}\approx 0\)) without introducing a stiff penalty spring. Continuous friction (when enabled) uses the same smoothed Coulomb form as \cref{sec:contacts-penalty}, but with this \(F_n\).

\paragraph{Hybrid interval integration in the coupled multirotor plant.}
...
\begin{equation}
  \vect{v}^{+}_{t} =
  \begin{cases}
    \vect{0}, & v^{-}_{t} \le \Delta v_{t,\max} \quad (\text{sticking impact}),\\[4pt]
...
\subparagraph{Grounded time-stepping loop.}
...
\emph{3) Position-only post-stabilization.}
...
Because impacts are applied as discrete velocity jumps, the spike appears in
\code{impact\_accel\_est} (and \code{impact\_dv}) but not in the continuous acceleration or
specific-force channels (\code{acc\_x/y/z}, \code{spec\_bx/by/bz}), which reflect the smooth
dynamics evaluated at log boundaries.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Penalty model (\code{FlatGroundContact}).} Keys map to the compliant spring--damper:
    \begin{itemize}[leftmargin=*,itemsep=1pt]
      \item \code{k\_n\_per\_m}: normal spring constant \(k\) in \(F_n = k\,\delta + c\,v_z\).
