...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \code{arm\_time\_s}: time at which the scenario requests arming (\(\texttt{vehicle\_control\_mode.armed}=true\)).
  \item \code{mission\_time\_s}: time at which mission start is requested (mapped to \code{vehicle\_status.nav\_state} transitions in the PX4 bridge).
\end{itemize}
