...
The PX4 integration is configured under \code{[px4]} (\code{PX4Spec} parsed in \code{src/sim/Aircraft/toml/Sections/PX4.jl}, included by \code{src/sim/Aircraft/TOMLIO.jl}). The same table also contains the lockstep module scheduling rates (\code{[px4.lockstep]}) and the uORB publish/subscribe interface definition (\code{[px4.uorb]}).
