...
The command packet drives a small internal mode logic that sets \code{nav\_state} and \code{arming\_state} fields for injected topics. A configurable \code{edge\_trigger} mode determines whether mission/RTL requests are treated as edges or levels; additional ``mission rearm'' logic exists for the level-triggered mode to make mission activation robust.
