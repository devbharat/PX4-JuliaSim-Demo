...
Simulation instances are built from an \code{AircraftSpec} (\code{src/sim/Aircraft/Spec.jl}) typically loaded from TOML via \code{Aircraft.load\_spec} (include hub in \code{src/sim/Aircraft/TOMLIO.jl}, with section parsers under \code{src/sim/Aircraft/toml/}). The spec layer exists to separate:
