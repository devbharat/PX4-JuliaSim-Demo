...
The test suite is designed to run without a PX4 shared library. Lockstep runtime smoke tests (\code{test/lockstep_runtime.jl}) are conditionally executed only when the lockstep library is available (see the \code{PX4Lockstep.has_lockstep()} gate).

\subsection{Analytic integrator verification}

The module \code{Sim.Verification} defines closed-form or invariant-rich reference problems, used in \code{test/verification_cases.jl}:
...
The file \code{test/verification_contracts.jl} contains deterministic ``contract'' tests for the full multirotor plant and power system:
\begin{itemize}[leftmargin=*,itemsep=2pt]
...
The record/replay machinery is covered by \code{test/record_replay_engine.jl}. These tests exercise:
\begin{itemize}[leftmargin=*,itemsep=2pt]
...
The uORB injection scheduler and default state injector are covered by \code{test/uorb_injection.jl}, including multi-instance battery publishing behavior and the periodic ``due'' logic.
