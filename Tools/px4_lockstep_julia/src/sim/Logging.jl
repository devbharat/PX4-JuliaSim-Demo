...
function SimLog()
    return SimLog(
        Float64[],
        NTuple{3,Float64}[], NTuple{3,Float64}[], NTuple{4,Float64}[], NTuple{3,Float64}[],
        NTuple{3,Float64}[], NTuple{3,Float64}[], NTuple{3,Float64}[], Float64[], Float64[],
        NTuple{4,Float64}[], NTuple{12,Float64}[],
        NTuple{3,Float64}[], Float64[],
...
const _CSV_HEADER = join([
    ...
    "rho",
    # battery
    "batt_v","batt_a","batt_rem","batt_warn",
    # px4
    "nav_state","arming_state","mission_seq","mission_count","mission_finished",
], ",")
...
function log!(sink::CSVLogSink,
    ...
    # motors 1..12
    @printf(io, "%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,",
            m[1],m[2],m[3],m[4],m[5],m[6],m[7],m[8],m[9],m[10],m[11],m[12])
    # environment
    @printf(io, "%.6f,%.6f,%.6f,%.6f,", wind_ned[1], wind_ned[2], wind_ned[3], rho)
    # battery
    @printf(io, "%.6f,%.6f,%.6f,%d,", battery.voltage_v, battery.current_a, battery.remaining, battery.warning)
    # px4
    @printf(io, "%d,%d,%d,%d,%d\n", nav_state, arming_state, mission_seq, mission_count, mission_finished)
...
function write_csv(log::SimLog, path::AbstractString)
    open(path, "w") do io
        ...
        for i in 1:n
            ...
            @printf(io, "%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,",
                    m12[1],m12[2],m12[3],m12[4],m12[5],m12[6],m12[7],m12[8],m12[9],m12[10],m12[11],m12[12])

            @printf(io, "%.6f,%.6f,%.6f,%.6f,", wind[1], wind[2], wind[3], log.air_density[i])

            @printf(io, "%.6f,%.6f,%.6f,%d,", log.batt_voltage_v[i], log.batt_current_a[i],
                    log.batt_remaining[i], log.batt_warning[i])

            @printf(io, "%d,%d,%d,%d,%d\n", log.nav_state[i], log.arming_state[i],
                    log.mission_seq[i], log.mission_count[i], log.mission_finished[i])
        end
    end
    return nothing
end
